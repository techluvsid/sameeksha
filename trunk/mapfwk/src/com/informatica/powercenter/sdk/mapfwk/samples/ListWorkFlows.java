...
public class ListWorkFlows {
    ...
    public void execute() throws Exception {
        ...
        for(int i=0 ; i < folderSize; i++){
        	Vector listOfWorkFlows = ((Folder)folders.get(i)).getWorkFlows(); //get the list of workflows
        	int listSize = listOfWorkFlows.size();
			System.out.println(" ***** List of workflows ******");
        	for(int j=0; j < listSize; j++){
        		System.out.println(((Workflow)listOfWorkFlows.get(j)).getName());
        	}
        }
    }
    ...
    public static void main(String[] args) {
        try {
        	ListWorkFlows repo = new ListWorkFlows();
            repo.execute();
        } catch (Exception e) {
            e.printStackTrace();
