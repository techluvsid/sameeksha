/*
 * MappletSorter.java Created on Nov 4, 2005.
 *
...
public class MappletSorter extends Base {
    ...
    public void createMappings() throws Exception {
        // create a mapping
        mapping = new Mapping( "MappletSorter", "MappletSorter",
                "This is sample for Mapplet Sorter" );
        ...
        // with name "testSorter" in the
        // Power Center Repository.
        RowSet dsqRS = (RowSet) helper.mapplet( vec, TransformationConstants.MAPPLET, "testSorter" )
                .getRowSets().get( 0 );
    ...
    public static void main( String args[] ) {
        try {
            MappletSorter mapSorter = new MappletSorter();
            if (args.length > 0) {
                if (mapSorter.validateRunMode( args[0] )) {
                    mapSorter.execute();
                }
            } else {
                mapSorter.printUsage();
            }
        } catch (Exception e) {
            e.printStackTrace();
